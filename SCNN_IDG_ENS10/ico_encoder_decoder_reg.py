...
class IcoEncoderDecoder(BaseSegmentor):
    ...
    def predict(self,
        ...
        """
        if data_samples is not None: # test/val 处理的时候可能需要用到每个样本的处理信息 从datasampler中提取出来 居然没有函数 还要显示调用。
            batch_img_metas = [
    ...
    def inference(self, inputs: Tensor, batch_img_metas: List[dict]) -> Tensor:
        """Inference with slide/whole style.
            test/val 需要运行的主函数 必须有 ori_shape 这个metainfo才行 因此在PackIcoInputs中需要加上
        Args:
