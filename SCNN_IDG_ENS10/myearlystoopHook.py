...
class MyEarlyStoppingHook(Hook):
    ...
    def _check_stop_condition(self, current_score: float) -> Tuple[bool, str]:
        ...
        if self.check_finite and not isfinite(current_score):
            ...
            # 添加更新信息 2023 1026 发现最后输出的self.best_score 和val中保存的best_score不一致，所以添加了这个验证怎么回事
            reason_message = (f'update best score: {current_score:.3f} ori score {self.best_score:.3f}. ')
            self.best_score = current_score
            self.wait_count = 0
